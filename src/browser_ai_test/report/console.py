...
def render_case(result: CaseResult, index: int, total: int) -> None:
    ...
    table.add_column("UI"); table.add_column("Network"); table.add_column("Answer")
    table.add_column("TTFT"); table.add_column("Stream"); table.add_column("Result")
    table.add_row(status(result.ui_ok), status(result.network_ok), status(result.answer_ok),
...
def render_summary(run_id: str, stats: dict[str, Any]) -> None:
    ...
    console.print(f"UI Success: {stats['ui_success_rate']:.2f}%  Network Success: {stats['network_success_rate']:.2f}%  Answer Success: {stats['answer_success_rate']:.2f}%")
